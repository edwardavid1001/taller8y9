...
public class UserRepositoryImpl implements co.edu.sena.project_2687365.model.repository.Repository<User> {
    ...
    public Integer saveObj(User user) throws SQLException {
        ...
        if (user.getUser_id()!=null && user.getUser_id()>0){
            sql = "update users_tbl set user_firstname=?, user_lastname=?,user_email=?,"+
                    "user_password= aes_encrypt(?,'$2a$12$FljPSuR9kAl9ly5g27x.7.rwRyjgSIl40tuOr6dKn61xlV6Gh0BOW')"
                    +" where user_id=?";

        } else {
            sql = "insert into users_tbl(user_firstname,user_lastname,user_email,user_password)"+
                    "values(upper(?),upper(?),upper(?),"+"aes_encrypt(?,'$2a$12$FljPSuR9kAl9ly5g27x.7.rwRyjgSIl40tuOr6dKn61xlV6Gh0BOW'))";
        }
        try(Connection conn =ConnectionPool.getConnection();
            PreparedStatement ps = conn.prepareStatement(sql)) {
